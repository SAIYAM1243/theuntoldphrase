...
export const useAnalytics = (toast, toggle, token) => {
    return useMutation({
        mutationFn: async (val) => {
            ...
            const { data } = await axios.post(`${API_URI}/posts/admin-analytics?query=${val}`, null, {
                headers: {
...
export const useCreatePost = (toast, toggle, token) => {
    return useMutation({
        mutationFn: async (formData) => {
            ...
            const { data } = await axios.post(`${API_URI}/posts/create-post`, formData, {
                headers: {
                    Authorization: `Bearer ${token}`,
                },
            }
...
export const useContent = (toast, toggle, token) => {
    return useMutation({
        mutationFn: async (page) => {
            ...
            const { data } = await axios.post(`${API_URI}/posts/admin-content?page=${page}`, null, {
                headers: {
                    Authorization: `Bearer ${token}`,
                },
            }
...
export const useAction = (toast, token) => {
    return useMutation({
        mutationFn: async ({ id, status }) => {
            ...
            const { data } = await axios.patch(`${API_URI}/posts/update/` + id, { status: status }, {
                headers: {
                    Authorization: `Bearer ${token}`,
                },
            }
...
export const useDeleteComment = (token) => {
    return useMutation({
        mutationFn: async ({ id, postId }) => {
            const { data } = await axios.delete(`${API_URI}/posts/comment/${id}/${postId}`,
                {
                    headers: {
                        Authorization: `Bearer ${token}`,
                    },
