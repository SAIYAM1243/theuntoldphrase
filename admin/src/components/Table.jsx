            ...
            >
                {
                    follower?.image ? (
                        ...
                        <p
                            className="w-10 h-10 rounded-full flex justify-center items-center first:bg-blue-700 text-white"
                        >
...
export const RecentPostTable = ({ data, theme }) => {
    const tableData = data?.map((el) => (
        <Table.Tr
            key={el?._id}
            className={theme ? "text-gray-400" : "text-slat-600"}
        >
            <Table.Td
                className="flex gap-2 items-center"
            >
